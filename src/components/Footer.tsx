        ...
        <section className="flex flex-col gap-3 pt-10 ">
            ...
            <section className="flex md:flex-row flex-col justify-between gap-5">
                <section className="flex flex-col gap-2 items-center">
                    <button className={`cursor-pointer font-semibold text-4xl text-transparent bg-clip-text bg-linear-to-r from-[#42CBA2] to-[#1C609E] ${cherry_bomb_one.className}`}>SMART</button>
                    ...
                    <div className="flex flex-col items-center">
                        ...
                        {data.map((item: NavbarItem) => (
                            <Link key={item.id} href={`/${item.name}`} className="flex items-center p-1">
                                <button className="cursor-pointer hover:underline decoration-[#ca4520] py-3;">{item.name}</button>
                            </Link>
                        ))}
